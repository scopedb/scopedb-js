# Node.js SDK Delivery Notes

This document captures the Node.js package-delivery concerns that sit adjacent to the SDK implementation itself.
...
- `.gitignore` ignores generated artifacts:
  ...
  - `dist-test/`
  - `node_modules/`
- `examples/` now provides minimal statement / table / batch examples.
- `README.md` now points at the in-repo examples and documents the current development commands.
...
The package already points `types` at `dist/index.d.ts`, but the final delivery shape should be reviewed as a Node package rather than just a TypeScript repo:
...
Before the package is actually published, add a build hook so npm tarballs always contain fresh runtime and declaration artifacts.
...
If a separate Node-specific workflow is introduced later, these commands should be the first candidates.

### 5. Revisit example execution ergonomics

The current examples are intentionally close to `src/` and serve as in-repo reference code. If the package becomes more user-facing, decide whether examples should instead run against built `dist/` output or a dedicated example runner.
